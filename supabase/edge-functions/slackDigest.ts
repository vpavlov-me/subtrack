...
export const handler = async (_req: Request): Promise<Response> => {
  try {
    ...
    if (!subscriptions || subscriptions.length === 0) {
      return new Response(JSON.stringify({ message: 'No upcoming payments to report' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
    }
    ...
    for (const [userId, userSubs] of Object.entries(grouped)) {
      ...
      if (!profile || !prefs || prefs.channel !== 'slack' || !prefs.slack_webhook_url) continue
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error(`Failed to send Slack message to ${profile.email}:`, response.statusText)
        }
      } catch (error) {
        console.error(`Error sending Slack message to ${profile.email}:`, error)
      }
    }
