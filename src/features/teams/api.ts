...
export type TeamMember = { team_id: string; member_id: string; role: 'owner'|'admin'|'member'; active: boolean }
...
export async function inviteMember(teamId: string, email: string, role: 'admin'|'member' = 'member') {
  const { error } = await supabase.functions.invoke('send-invite-email', { body: { teamId, email, role } })
  if (error) throw error
}

export async function updateMemberRole(teamId: string, memberId: string, role: 'admin'|'member') {
  await supabase.from('team_members').update({ role }).eq('team_id', teamId).eq('member_id', memberId)
}
