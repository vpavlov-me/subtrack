...
import { fetchSubscriptions, addSubscription as add, updateSubscription as update, deleteSubscription as remove } from './api'
...
export function SubscriptionsProvider({ children }: { children: ReactNode }) {
  ...
  const [billingCycleFilter, setBillingCycleFilter] = useState<'monthly' | 'yearly' | 'all'>('all')
  ...
  const totals = React.useMemo(() => {
    const monthly = subscriptions.reduce((sum, sub) => sum + (sub.billingCycle === 'monthly' ? sub.price : sub.price / 12), 0)
    const yearly = subscriptions.reduce((sum, sub) => sum + (sub.billingCycle === 'yearly' ? sub.price : sub.price * 12), 0)
