...
import { parseCsvSubscriptions, generateCsvTemplate, validateCsvHeaders } from './csv'
...
Netflix,15.99,Entertainment`
      ...
      expect(result.errors).toContain('Missing required headers: billingcycle, nextbillingdate')
    })
...
Netflix,invalid,monthly,2024-02-01`
      ...
      expect(result.success).toBe(false)
      expect(result.errors).toContain('Row 1: Invalid price "invalid". Must be a positive number.')
    })
...
Netflix,15.99,weekly,2024-02-01`
      ...
      expect(result.success).toBe(false)
      expect(result.errors).toContain('Row 1: Invalid billing cycle "weekly". Must be monthly, yearly, or custom.')
    })
...
Netflix,15.99,monthly,invalid-date`
      ...
      expect(result.success).toBe(false)
      expect(result.errors).toContain('Row 1: Invalid date "invalid-date". Use YYYY-MM-DD format.')
    })
...
Netflix,15.99,monthly,2024-02-01,XYZ`
      ...
      expect(result.success).toBe(true)
      expect(result.warnings).toContain('Row 1: Unsupported currency "XYZ". Using USD instead.')
...
Netflix,15.99,monthly,2024-02-01`
  ...
  describe('generateCsvTemplate', () => {
    it('should generate valid CSV template', () => {
      ...
      expect(template).toContain('Name,Price,Billing Cycle,Next Billing Date,Category,Currency,Payment Method,Notes')
      expect(template).toContain('Netflix,15.99,monthly,2024-02-01,Entertainment,USD,Credit Card,Personal account')
    })
  })
  ...
  describe('validateCsvHeaders', () => {
    ...
    it('should detect missing headers', () => {
      ...
      expect(result.errors).toContain('Missing required header: nextbillingdate')
    })
