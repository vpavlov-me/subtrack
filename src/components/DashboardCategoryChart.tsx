  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const { data, error } = await supabase
        .rpc('get_user_category_analytics', { user_id_param: user.id })
      ...
      const formattedData = data?.map((item: CategoryData) => ({
        ...item,
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium">
              {formatAmount(
                categoryData.reduce((sum, item) => sum + item.monthly_revenue, 0),
                userCurrency
              )}
            </p>
          </div>
